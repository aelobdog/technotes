...
- this technique basically represents an entire block of the source text as a single floating point number, which can be used to obtain the source text by repeating the same zooming process till the end of stream symbol is encountered
...
- the dictionary is filled as the cursor moves from the begginging of the source text to the end, using only the characters/patterns visited till that point
